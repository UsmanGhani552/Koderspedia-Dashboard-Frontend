...
const CustomersOverviewTwo = () => {
  // let { donutChartSeries, donutChartOptions } = useReactApexChart();
  const { invoices } = useSelector((state) => state.invoices);
  const paidInvoices = invoices.filter(invoice => invoice.status == 1).length;
  const unPaidInvoices = invoices.filter(invoice => invoice.status == 0).length;
...
export default CustomersOverviewTwo;